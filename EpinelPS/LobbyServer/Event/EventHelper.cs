    ...
    private static void AddEvents(ref ResGetEventList response, List<NetEventData> eventDatas)
    {
        foreach (var eventData in eventDatas)
        {
            ...
            if (!response.EventList.Any(e => e.Id == eventData.Id))
            {
                ...
                {
                    // Any event whose banner table is NOT perm_lobby is a dated or historical event banner
                    // whose localized sprite table is not present in the client's localized sprite databases.
                    // Mask carousel visibility dates to the past (Ticks = 1) so LobbyBannerListAdapter never
                    // attempts to load non-existent localized sprite tables (preventing FileNotFoundException).
                    // EventStartDate and EventEndDate remain active so the event is 100% playable via the private banner.
                    if (em.EventBannerResourceTable != "perm_lobby")
                    {
